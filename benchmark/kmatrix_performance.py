...
def benchmark_kmatrix(array_length, n_channels, n_poles, n_runs=5):
    ...
    if config.backend_name == "jax":
        ...
        def kmat(*args, **kwargs):
            return kmat_jitted(s_values, *args, **kwargs)
...
def run_full_benchmark():
    ...
    for array_length, n_channels, n_poles in product(array_lengths, n_channels_range, n_poles_range):
        combination_count += 1
        print(
            f"[{combination_count:2d}/{total_combinations}] "
            f"Array: {array_length:5d}, Channels: {n_channels}, Poles: {n_poles} ... ",
            end="",
            flush=True,
        )
...
def analyze_results(df):
    ...
    for _, row in fastest.iterrows():
        print(
            f"  Array: {row['array_length']:5d}, Channels: {row['n_channels']}, "
...
def create_plots(df):
    ...
    for n_channels in sorted(valid_df["n_channels"].unique()):
        for n_poles in sorted(valid_df["n_poles"].unique()):
            subset = valid_df[(valid_df["n_channels"] == n_channels) & (valid_df["n_poles"] == n_poles)]
            if len(subset) > 0:
                label = f"{n_channels}ch, {n_poles}p"
                ax1.plot(subset["array_length"], subset["mean_time_ms"], "o-", label=label, alpha=0.7)
    ...
    for array_length in array_lengths_to_plot:
        ...
        if len(subset) > 0:
            ...
            ax2.plot(grouped.index, grouped.values, "o-", label=f"{array_length} points", alpha=0.7)
    ...
    for array_length in array_lengths_to_plot:
        ...
        if len(subset) > 0:
            ...
            ax3.plot(grouped.index, grouped.values, "o-", label=f"{array_length} points", alpha=0.7)
    ...
    if len(heatmap_data) > 0:
        ...
        pivot_table = heatmap_data.pivot_table(values="mean_time_ms", index="n_poles", columns="n_channels", aggfunc="mean")
        ...
        for i in range(len(pivot_table.index)):
            for j in range(len(pivot_table.columns)):
                if not np.isnan(pivot_table.iloc[i, j]):
                    ax4.text(j, i, f"{pivot_table.iloc[i, j]:.1f}", ha="center", va="center", color="white", fontweight="bold")
